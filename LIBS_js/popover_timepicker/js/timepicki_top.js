		...
		var defaults = {
			format_output: function(tim, mini, meri) {
			    if (settings.show_meridian) {
			        ...
			        tim = Math.min(Math.max(parseInt(tim), 0), 9999);
		...
		return this.each(function() {
			...
			var new_ele = $(
				"<div class='timepicker_wrap " + settings.custom_classes + "'>" +
					...
					"<div class='time'>" +
						top_arrow_button +
						"<div class='ti_tx'><input type='text' class='timepicki-input' style='margin-right: 0px !important;'></div>" +
						bottom_arrow_button +
					"</div>" +
					"<div class='mins'>" +
						top_arrow_button +
						"<div class='mi_tx'><input type='text' class='timepicki-input' style='margin-right: 0px !important;'></div>" +
						bottom_arrow_button +
					"</div>");
			if(settings.show_meridian){
				new_ele.append(
			    ...
			    function validate() {
			        ...
			        if (grandfatherDiv.hasClass("time")) { /// HOUR
			            ...
			            if (isValidNumber) {
			                ...
			                Math.min(Math.max(parseInt(input.val()), 0), 9999); // for 24 hours date picker.
